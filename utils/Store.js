...
function reducer(state, action) {
  ...
  switch (
    ...
    case "CART_ADD_ITEM": {
      // 5. y en caso de que sea igual a CART_ADD_ITEM, haga lo siguiente
      const newItem = action.payload; //primero creo una variable con lo que haiga en action.payload
      const existItem = state.cart.cartItems.find(
        //segundo creo una variable y busco en el estado
      ...
      const cartItems = existItem //Creo una variable y si existe el item
        ? state.cart.cartItems.map(
            (
              item //que me haga un recorrido del estado
...
export function StoreProvider({ children }) {
  //ahora si creo mi componente StoreProvider y que los hijos puedan acceder
